...
export const AgencySidebar: React.FC<AgencySidebarProps> = ({ activeView, onViewChange }) => {
    ...
    const menuItems = [
        ...
        { id: 'notifications', label: 'Notificaciones', icon: <PaperAirplaneIcon className="w-5 h-5" /> },
            ...
            <div className="flex-1 py-6 px-3 space-y-1 overflow-y-auto">
                {menuItems.map(item => (
                    <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 min-h-[44px] rounded-lg text-sm font-medium transition-all duration-200
                            ${activeView === item.id
