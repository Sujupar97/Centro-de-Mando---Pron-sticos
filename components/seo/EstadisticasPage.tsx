    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between border-b border-white/5">
        ...
        <div className="flex items-center gap-4 text-sm">
          ...
          <Link to="/estadisticas" className="text-white font-medium">Estadísticas</Link>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <header className="text-center mb-12">
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-4 font-display">Track Record Verificado</h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-3xl mx-auto">
            <div className="bg-slate-900/80 border border-white/5 rounded-2xl p-5 backdrop-blur-xl">
              <div className="text-3xl font-bold text-emerald-400">{overall.win_rate}%</div>
              <div className="text-slate-500 text-sm mt-1">Efectividad Total</div>
            </div>
            <div className="bg-slate-900/80 border border-white/5 rounded-2xl p-5 backdrop-blur-xl">
              <div className="text-3xl font-bold text-white">{overall.total_picks}+</div>
            ...
            <div className="bg-slate-900/80 border border-white/5 rounded-2xl p-5 backdrop-blur-xl">
              ...
              <div className="text-slate-500 text-sm mt-1">Últimos 7 Días</div>
            </div>
            <div className="bg-slate-900/80 border border-white/5 rounded-2xl p-5 backdrop-blur-xl">
              <div className="text-3xl font-bold text-white">{stats?.totalLeagues || 0}+</div>
              <div className="text-slate-500 text-sm mt-1">Ligas</div>
            </div>
          </div>
        </header>

        {/* Period comparison */}
        <section className="bg-slate-900/80 border border-white/5 rounded-2xl p-6 mb-8 backdrop-blur-xl">
          <h2 className="text-xl font-bold text-white mb-4 font-display">Efectividad por Período</h2>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="text-slate-300">
                <tr className="border-b border-white/5">
                  ...
                  <td className="py-3"><span className={`font-bold ${p7d.win_rate >= 60 ? 'text-emerald-400' : 'text-white'}`}>{p7d.win_rate}%</span></td>
                </tr>
                <tr className="border-b border-white/5">
                  <td className="py-3 pr-4">Últimos 30 días</td>
                  <td className="py-3 pr-4">{p30d.total}</td>
                  <td className="py-3 pr-4">{p30d.won}</td>
                  <td className="py-3"><span className={`font-bold ${p30d.win_rate >= 60 ? 'text-emerald-400' : 'text-white'}`}>{p30d.win_rate}%</span></td>
                </tr>
                <tr>
                  ...
                  <td className="py-3"><span className={`font-bold ${overall.win_rate >= 60 ? 'text-emerald-400' : 'text-white'}`}>{overall.win_rate}%</span></td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        {stats?.trend && stats.trend.length > 0 && (
          <section className="bg-slate-900/80 border border-white/5 rounded-2xl p-6 mb-8 backdrop-blur-xl">
            <h2 className="text-xl font-bold text-white mb-4 font-display">Tendencia de Efectividad (30 Días)</h2>
        ...
        <div className="bg-slate-900/80 border border-white/5 rounded-2xl p-8 mb-8 text-center backdrop-blur-xl">
          <h3 className="text-xl font-bold text-white mb-2">Accede a Pronósticos con Esta Efectividad</h3>
          <p className="text-slate-400 mb-4">1 pronóstico gratis al día, para siempre. Sin tarjeta de crédito.</p>
        ...
        {stats?.byMarket && stats.byMarket.length > 0 && (
          <section className="bg-slate-900/80 border border-white/5 rounded-2xl p-6 mb-8 backdrop-blur-xl">
            <h2 className="text-xl font-bold text-white mb-4 font-display">Efectividad por Tipo de Mercado</h2>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-slate-500 text-xs uppercase border-b border-white/5">
                ...
                <tbody className="text-slate-300">
                  {stats.byMarket.map((m) => (
                    <tr key={m.market} className="border-b border-white/5">
                      ...
                      <td className="py-3">
                        <span className={`font-bold ${m.win_rate >= 60 ? 'text-emerald-400' : m.win_rate >= 50 ? 'text-white' : 'text-red-400'}`}>
                          {m.win_rate}%
                        </span>
                      </td>
                    </tr>
        ...
        {stats?.byLeague && stats.byLeague.length > 0 && (
          <section className="bg-slate-900/80 border border-white/5 rounded-2xl p-6 mb-8 backdrop-blur-xl">
            <h2 className="text-xl font-bold text-white mb-4 font-display">Efectividad por Liga</h2>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-slate-500 text-xs uppercase border-b border-white/5">
                ...
                <tbody className="text-slate-300">
                  {stats.byLeague.map((l) => (
                    <tr key={l.league} className="border-b border-white/5">
                      <td className="py-3 pr-4">{l.league}</td>
                      <td className="py-3 pr-4">{l.total}</td>
                      <td className="py-3 pr-4">{l.won}</td>
                      <td className="py-3">
                        <span className={`font-bold ${l.win_rate >= 60 ? 'text-emerald-400' : l.win_rate >= 50 ? 'text-white' : 'text-red-400'}`}>
                          {l.win_rate}%
                        </span>
                      </td>
                    </tr>
        ...
        {/* Methodology */}
        <section className="bg-slate-900/80 border border-white/5 rounded-2xl p-6 mb-8 backdrop-blur-xl">
          <h2 className="text-xl font-bold text-white mb-4 font-display">Metodología</h2>
