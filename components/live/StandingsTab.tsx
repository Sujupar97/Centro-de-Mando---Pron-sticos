        ...
        <div className="overflow-x-auto bg-gray-800 rounded-lg">
            <table className="w-full text-sm text-left min-w-[500px]">
                <thead className="bg-gray-700/50 text-gray-400 uppercase text-xs">
                    <tr>
                        {headers.map(h => <th key={h} className="px-2 py-2 sm:p-3 font-semibold">{h}</th>)}
                    </tr>
                </thead>
                    ...
                    {standings.map(s => {
                        ...
                        return (
                            <tr key={s.rank} className={rowClass}>
                                <td className="px-2 py-2 sm:p-3 font-bold text-white">{s.rank}</td>
                                <td className="px-2 py-2 sm:p-3 flex items-center whitespace-nowrap">
                                    <img src={s.team.logo} alt={s.team.name} className="w-5 h-5 mr-2 sm:mr-3" />
                                    <span className="font-semibold text-white">{s.team.name}</span>
                                </td>
                                <td className="px-2 py-2 sm:p-3">{s.all.played}</td>
                                <td className="px-2 py-2 sm:p-3">{s.all.win}</td>
                                <td className="px-2 py-2 sm:p-3">{s.all.draw}</td>
                                <td className="px-2 py-2 sm:p-3">{s.all.lose}</td>
                                <td className="px-2 py-2 sm:p-3">{s.goalsDiff}</td>
                                <td className="px-2 py-2 sm:p-3 font-bold text-white">{s.points}</td>
                            </tr>
                        );
                    })}
                </tbody>
