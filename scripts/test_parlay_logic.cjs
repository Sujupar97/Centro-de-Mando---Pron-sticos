...
async function verifyLinkage(date) {
    ...
    // If 0 found, list some fixture_ids from picks > 0.80 for any date to see format
    if (linkedPicks.length === 0) {
        console.log("Found 0 linked picks. Checking ANY high prob picks...");
        ...
        if (anyError) console.error("Any Picks Error:", anyError);
            ...
            anyPicks.forEach(p => console.log(`Pick ${p.id}: Fixture ${p.fixture_id} (Type: ${typeof p.fixture_id}), Prob ${p.p_model}`));

            // Check type mismatch?
            const sampleMatchId = matchIds[0];
            console.log(`Sample Match ID: ${sampleMatchId} (Type: ${typeof sampleMatchId})`);
        }
    } else {
        // Show sample linked pick
        const sample = linkedPicks[0];
        console.log(`Success! Linked pick: ${sample.id}, Fixture: ${sample.fixture_id}, Prob: ${sample.p_model}`);
    }
}
