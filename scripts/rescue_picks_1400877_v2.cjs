...
async function rescuePicks() {
    console.log("Starting Rescue Mission for Report 19451998 -> Fixture 1400877 (RETRY with Confidence)");
    ...
    // 2. Map Picks with CONFIDENCE this time
    const picksToInsert = packet.pronosticos.map(p => ({
        job_id: report.job_id,
        fixture_id: 1400877, // CRITICAL FIX: Use the daily_matches ID
        market: p.mercado,
        selection: p.seleccion,
        p_model: p.probabilidad_calculada_porcentaje / 100,
        decision: p.probabilidad_calculada_porcentaje >= 75 ? 'BET' : (p.probabilidad_calculada_porcentaje >= 60 ? 'OBSERVAR' : 'EVITAR'),
        confidence: p.confianza || 'MEDIA', // REQUIRED FIELD
        odds: p.cuota_actual,
        created_at: new Date().toISOString()
    }));

    console.log(`Found ${picksToInsert.length} picks to insert:`);
    picksToInsert.forEach(p => console.log(`- ${p.market} ${p.selection} (${p.p_model}) Confidence: ${p.confidence}`));

    // 3. Clear existing picks for this fixture (to avoid duplicates if any exists under this ID)
    ...
    if (insertError) {
        ...
        console.log("Matches table ID: 1400877");
    }
}
