/**
 * STANDALONE DIAGNOSTIC: Dump Raw AI Output
...
async function main() {
    console.log('\n' + '═'.repeat(80));
    console.log('🔬 RAW AI OUTPUT DIAGNOSTIC (STANDALONE)');
    ...
    if (!reports || reports.length === 0) {
        console.log('❌ No reports_v2 record found');
        return;
    }
    ...
    if (packet.pronosticos?.length > 0) {
        ...
        console.log('   ❌ pronosticos: EMPTY');
    }

    if (packet.probabilidades_derbix) {
        console.log('   ⚠️  probabilidades_derbix (alternate format):');
