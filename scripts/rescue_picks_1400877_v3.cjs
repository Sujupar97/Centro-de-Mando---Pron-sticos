...
async function rescuePicks() {
    console.log("Starting Rescue Mission for Report 19451998 -> Fixture 1400877 (RETRY with Conf Integer)");

    const updates = [
        {
            job_id: 'fd7cb6a4-c304-4d5c-8e44-b37787dd78be', // Placeholder, using actual logic below
            fixture_id: 1400877,
        }
    ];
    ...
    // 2. Map Picks with INTEGER CONFIDENCE
    ...
    const picksToInsert = packet.pronosticos.map(p => ({
        ...
        confidence: mapConfidence(p.confianza), // INTEGERS
        odds: p.cuota_actual,
