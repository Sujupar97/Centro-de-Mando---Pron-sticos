...
async function rescuePicks() {
    console.log("Starting Rescue Mission for Report 19451998 -> Fixture 1400877 (RETRY with engine_version)");
    ...
    // 2. Map Picks with Correct Types
    ...
    const picksToInsert = packet.pronosticos.map(p => ({
        ...
        engine_version: "V4-RESCUE", // REQUIRED
        odds: p.cuota_actual,
