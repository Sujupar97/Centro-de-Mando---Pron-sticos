...
export type PredictionReviewStatus = 'auto_approved' | 'pending_review' | 'admin_approved' | 'admin_rejected';

export interface TopPickItem { predictionId?: string; gameId: number; analysisRunId?: string; matchup: string; date: string; league: string; teams: { home: { name: string; logo: string }; away: { name: string; logo: string }; }; bestRecommendation: BettingRecommendationVisual; result?: 'Won' | 'Lost' | 'Pending' | 'Void'; odds?: number; alternative?: { market: string; probability: number; }; reviewStatus?: PredictionReviewStatus; suspiciousReason?: string | null; }
